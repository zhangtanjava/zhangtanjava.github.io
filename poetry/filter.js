...
const displayTrans = (trans) =>{
    productsContainer.innerHTML = trans.map(tran=>
        `
	<div class="product">
		<img 
			src=${tran.headPhoto} 
			alt=""
		/>
		<span class="name">${tran.leagueName}</span>
		<span class="priceText">$${tran.playerRole}</span>
	</div>
	`
    ).join("");
};
...
function getAjax(value){
    ...
    xhr.onreadystatechange = function(){
        if (this.readyState == 4 && this.status == 200){
            ...
            const resCode=json.resCode
            const resMessage=json.resMessage
            const trans = json.retData
            displayTrans(trans)
            console.log(resCode)
        }
    }
